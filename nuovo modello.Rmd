...
```{r}
coda::autocorr.plot(beta_post_mcmc)
...
```{r}
geweke_beta <- apply(beta_post_mcmc, 2, function(x) geweke.diag(mcmc(x)))
geweke_sigma2 <- geweke.diag(mcmc(sigma2_post_mcmc))

# Estrai i valori z
z_scores_beta <- sapply(geweke_beta, function(x) x$z)
z_scores_sigma2 <- geweke_sigma2$z

# Convertire i risultati in data frame per ggplot2
df_beta <- data.frame(Parameter = paste0("Beta[", 1:6, "]"), Z_Score = z_scores_beta)
df_sigma2 <- data.frame(Parameter = "Sigma2", Z_Score = z_scores_sigma2)
...
all.marg.like = exp(all.log.marg.like - const)  # Usa una costante per la stabilità numerica
post.model.probs = all.marg.like / sum(all.marg.like)
